...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#Step 5: This step isn't necessary for this example, because Kelly Meredith already}
\CommentTok{#   groomed the values.  If the negative values (which represent NLSY missing or}
...
\NormalTok{)}
\KeywordTok{summary}\NormalTok{(dsDouble) }\CommentTok{#Notice there are 22176=(2*11088) records now (two for each unique pair).}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dsOutcomes <-}\StringTok{ }\KeywordTok{RenameNlsyColumn}\NormalTok{(dsOutcomes, }\StringTok{"C0328600"}\NormalTok{, }\StringTok{"BirthWeightInOunces"}\NormalTok{)}

\CommentTok{#Step 6: For this item, a negative value indicates the parent refused, didn't know,}
\CommentTok{#   invalidly skipped, or was missing for some other reason.}
\CommentTok{#   For our present purposes, we'll treat these responses equivalently.}
\CommentTok{#   Then clip/Winsorized/truncate the weight to something reasonable.}
...
\item
  ...
  rows and columns.
\item
  Estimate the SEM with the \pkg{lavaan} package. The function returns
...
\item
  ...
  \pkg{lavaan} package has additional methods that may be useful for
  your purposes.
\end{enumerate}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{### R Code for Example lavaan estimation analysis with a simple outcome and Gen2 subjects}
...
all \Sexpr{scales::comma(nrow(Links79Pair))} relationships in the follow
five types of NLSY79 relationships.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{xt <-}\StringTok{ }\NormalTok{xtable}\OperatorTok{::}\KeywordTok{xtable}\NormalTok{(}\KeywordTok{table}\NormalTok{(Links79Pair}\OperatorTok{$}\NormalTok{RelationshipPath, }\DataTypeTok{dnn=}\KeywordTok{c}\NormalTok{(}\StringTok{"Relationship Frequency"}\NormalTok{)),}
             \DataTypeTok{caption=}\StringTok{"Number of NLSY79 relationship, by `RelationshipPath`.(Recall that 'AuntNiece' also contains uncles and nephews.)"}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{### R Code for Example lavaan estimation analysis with a simple outcome and Gen1 subjects}
\CommentTok{#Steps 1-5 are explained in the vignette's first example:}
\KeywordTok{library}\NormalTok{(NlsyLinks)}
