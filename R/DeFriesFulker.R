...
DeFriesFulkerMethod1 <- function( dataSet, oName_S1, oName_S2, rName="R" ) { 
  lmDetails <- stats::lm(dataSet[, oName_S1] ~ 1 + dataSet[, oName_S2] + dataSet[, rName] + dataSet[, oName_S2]*dataSet[, rName])
...
DeFriesFulkerMethod3 <- function( dataSet, oName_S1, oName_S2, rName="R" ) { 
  ...
  lmDetails <- stats::lm(dv_S1Centered ~ 0 + dv_S2Centered + interaction) #The '0' specifies and intercept-free model.
  brief <- summary(lmDetails)
