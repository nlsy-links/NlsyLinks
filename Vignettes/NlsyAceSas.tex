...
\documentclass[letterpaper]{article}\usepackage{graphicx, color}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcomment{### R Code for Example lavaan estimation analysis with a simple outcome and Gen1 subjects}
\hlcomment{#Steps 1-5 are explained in the vignette's first example:}
\hlfunctioncall{require}(NlsyLinks) 
dsLinking <- \hlfunctioncall{subset}(Links79Pair, RelationshipPath==\hlstring{"Gen1Housemates"})
dsOutcomes <- ExtraOutcomes79 
\hlcomment{#The HeightZGenderAge variable is already groomed}

\hlcomment{#Step 6: Create the single entered dataset.}
dsSingle <- \hlfunctioncall{CreatePairLinksSingleEntered}(outcomeDataset=dsOutcomes, 
  linksPairDataset=dsLinking, outcomeNames=\hlfunctioncall{c}(\hlstring{'HeightZGenderAge'}))

\hlcomment{#Step 7: Declare the names for the two outcome variables. }
oName_1 <- \hlstring{"HeightZGenderAge_1"} 
oName_2 <- \hlstring{"HeightZGenderAge_2"} 
  
\hlcomment{#Step 8: Summarize the R groups and determine which groups can be estimated.}
dsGroupSummary <- \hlfunctioncall{RGroupSummary}(dsSingle, oName_1, oName_2)
rGroupsToDrop <- \hlfunctioncall{c}( 0 )
dsGroupSummary[dsGroupSummary$R %in% rGroupsToDrop, \hlstring{"Included"}] <- FALSE
dsGroupSummary
\end{alltt}
\begin{verbatim}
##       R Included PairCount O1Variance O2Variance O1O2Covariance
## 1 0.125     TRUE        88     0.9118     0.9502         0.1503
## 2 0.250     TRUE       238     1.0110     1.1427         0.2613
## 3 0.500     TRUE      3392     0.9660     1.0158         0.4376
## 4 1.000     TRUE        11     0.2909     0.6106         0.3747
##   Correlation Determinant PosDefinite
## 1      0.1614     0.84381        TRUE
## 2      0.2431     1.08697        TRUE
## 3      0.4417     0.78982        TRUE
## 4      0.8892     0.03719        TRUE
\end{verbatim}
\begin{alltt}
...
## [1] "Results of ACE estimation: [show]"
##  ASquared  CSquared  ESquared CaseCount 
## 8.956e-01 1.940e-14 1.044e-01 3.729e+03
\end{verbatim}
\begin{alltt}

\hlcomment{#Step 11: Inspect the output further (see the final step in the previous example).}
\end{alltt}
\end{kframe}
\end{knitrout}
