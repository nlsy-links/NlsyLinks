...
\item Open the zip file and extract the *.csv file to a location that the vignette examples have permission to read.  Then try some of the \code{NlsyLinks} vignette examples at \url{https://cran.r-project.org/package=NlsyLinks}. 
