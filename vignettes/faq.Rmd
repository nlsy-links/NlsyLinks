...
Welcome to the `NlsyLinks` package, which facilitates research with the [NLSY](https://www.bls.gov/nls/home.htm).  The initial focus of the package was to assist behavior genetics, but it has been expanded to help with NLSY research in general.
...
The current [FAQ](https://cran.r-project.org/package=NlsyLinks/vignettes/Faq.pdf) and the [NLSY ACE vignette](https://cran.r-project.org/package=NlsyLinks/vignettes/NlsyAce.pdf) provide overview, and the [reference manual](https://cran.r-project.org/package=NlsyLinks/NlsyLinks.pdf) describe the package's syntax and functions; the documents are available on [CRAN](https://cran.r-project.org/package=NlsyLinks).

Literature targeting general behavior genetics is listed in the appendix of the [NLSY ACE vignette](https://cran.r-project.org/package=NlsyLinks/vignettes/NlsyAce.pdf).  Furthermore, [the articles and books involving the NLSY kinship links](http://nlsy-links.github.io/NlsyLinks/research_publications.html) provide more specialized information.
...
Any NLSY dataset extracted using the [NLS Investigator](https://cran.r-project.org/package=NlsyLinks/vignettes/NlsInvestigator.pdf) will include some mandatory fields, including race and gender.  For Gen1, these are the `R02147.00` and `R02148.00` variables; for Gen2, they are the `C00053.00` and `C00054.00` variables.

The [NlsyAce vignette](https://cran.r-project.org/package=NlsyLinks/vignettes/NlsyAce.pdf) describes how to incorporate extracts into \R{} for manipulation and analysis (*e.g.*, ``DF analysis with a univariate outcome from a Gen2 Extract'').  Those vignette example focused on incorporating outcomes.  The FAQ entry focuses on race and gender.
...
Finally, consider converting the numeric variables to [factor](http://stat.ethz.ch/R-manual/R-devel/library/base/html/factor.html) variables so your code will be more readable.
...
Make sure that `dsOutcomes` also has the `SubjectTag` variable; this will happen automatically if it was read into R using the `ReadCsvNlsy79Gen1()` or `ReadCsvNlsy79Gen2()` functions.  For more information about `SubjectTag`, please see the `Links79Pair` entry in the [reference manual](https://cran.r-project.org/package=NlsyLinks/NlsyLinks.pdf).
