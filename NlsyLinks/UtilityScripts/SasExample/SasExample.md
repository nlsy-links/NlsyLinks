...
Note that for this to run missing values must be `.` as SAS specifies them not `NA`, the default R missing value code.  The file `links.csv` with the path `E:/links.csv` can be exported from the [`NlsyLinks`](http://cran.r-project.org/web/packages/NlsyLinks/) R package with the following R code.
