...
If you wish to exclude additional groups from the analyses, Step 8 should change slightly.  For instance, to MZ twins, replace the two lines in Step 8 with the following four.  This is unlikely to be useful idea in the current example, and is more likely to be useful when using the \code{RFull} variable, which includes all values of \emph{R} we were able to determine.
...
\begin{itemize}\raggedright
  ...
  \item Base packages: base, datasets, graphics, grDevices, methods, stats,
    utils
  \item Other packages: knitr~1.5, lavaan~0.5-15, NlsyLinks~1.230
